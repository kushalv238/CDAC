...
function createData(planes) {
    ...
    planes.forEach((plane1, index1) => {
        ...
        const { colour: planeColour } = plane1.visibility.plane ? plane1.colour : { colour: "transparent" }
        const { colour: normalColour } = plane1.visibility.normal ? plane1.colour : { colour: "transparent" }
        ...
        if (c !== 0) {
            z_arb = [
                [calculateZArbitary(a, b, c, d, x_arb[0][0], y_arb[0][0], referncePoints), calculateZArbitary(a, b, c, d, x_arb[0][1], y_arb[0][1], referncePoints)],
